...
target_counter=0
kill_target_counter=0
#--------log files--------
rm $logdir/spro_log.txt 2>/dev/null
touch $logdir/spro_log.txt

rm -r try_kill_spro/ 2>/dev/null
mkdir try_kill_spro

rm $logdir/spro_status.txt 2>/dev/null
touch $logdir/spro_status.txt
#----------------------------------------
...
do
	...
	do
		...
		echo imok >$logdir/spro_status.txt
		...
		if (($gen_run != 0))
		then
			echo "процесс GenTarget убит. Завершаю spro"
			exit 0
		fi
		...
		then
			...
			Targets_kill_list=$(ls -tr try_kill_spro/ 2>/dev/null)
			...
			do
				...
				then
					echo -e "SPRO 0: \e[31mне удалось уничтожить \e[0mЦель ID:${target_params[$id_index+$number_of_params*$target_index]}"
					rm try_kill_spro/${target_params[$id_index+$number_of_params*$target_index]}
				fi
			done
			...
			then
                ...
                SPRODetection ${target_params[$x_coord_index+$number_of_params*$target_index]} ${target_params[$y_coord_index+$number_of_params*$target_index]}
                
                #Если цель являеется ББ и уже попадала в зону действия СПРО пытаемся уничножить
                if ((${target_params[$target_type_index+$number_of_params*$target_index]} == 1)) && ((${target_params[$already_seen_index+$number_of_params*$target_index]} == 1))
                then
					...
					touch "try_kill_spro/${target_params[$id_index+$number_of_params*$target_index]}"
					#на экран
					echo -e "SPRO 0: \e[93mпопытка уничтожить \e[0mЦель ID:${target_params[$id_index+$number_of_params*$target_index]}"
					#в лог
					echo -e "SPRO 0: \e[93mпопытка уничтожить \e[0mЦель ID:${target_params[$id_index+$number_of_params*$target_index]}" >>$logdir/spro_log.txt
	...
	then
		for killed_targets in $Targets_kill_list
		do
			echo -e "SPRO 0: \e[32mуничтожена \e[0mЦель ID:$killed_targets"
			#в лог
			echo -e "SPRO 0: \e[32mуничтожена \e[0mЦель ID:$killed_targets" >>$logdir/spro_log.txt
			rm try_kill_spro/$killed_targets  #убираем из папки истинно уничтоженные цели
		done
	fi
	...
	#echo $imok >$logdir/spro_ok_status.txt
