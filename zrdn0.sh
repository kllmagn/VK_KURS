...
target_counter=0
#---------------log files----------------
rm $logdir/zrdn0_log.txt 2>/dev/null
touch $logdir/zrdn0_log.txt

rm -r try_kill_zrdn0/ 2>/dev/null
mkdir try_kill_zrdn0

rm $logdir/zrdn0_status.txt 2>/dev/null
touch $logdir/zrdn0_status.txt
#----------------------------------------
...
do
	...
	do
		...
		echo imok >$logdir/zrdn0_status.txt
		...
		if (($gen_run != 0))
		then
			echo "процесс GenTarget убит. Завершаю zrdn0"
			exit 0
		fi
		...
		then
			...
			Targets_kill_list=$(ls -tr try_kill_zrdn0/ 2>/dev/null)
			...
			do
				...
				then
					echo -e "ZRDN 0: \e[31mне удалось уничтожить \e[0mЦель ID:${target_params[$id_index+$number_of_params*$target_index]}"
					rm try_kill_zrdn0/${target_params[$id_index+$number_of_params*$target_index]}
				fi
			done
			...
			then
                ...
                then
                    ...
                    ZRDN0Detection ${target_params[$x_coord_index+$number_of_params*$target_index]} ${target_params[$y_coord_index+$number_of_params*$target_index]}
                fi
                ...
                then
                    ...
                    then
						...
						touch "try_kill_zrdn0/${target_params[$id_index+$number_of_params*$target_index]}"
                        #на экран
                        echo -e "ZRDN 0: \e[93mпопытка уничтожить \e[0mЦель ID:${target_params[$id_index+$number_of_params*$target_index]}"
                        #в лог
                        echo -e "ZRDN 0: \e[93mпопытка уничтожить \e[0mЦель ID:${target_params[$id_index+$number_of_params*$target_index]}" >>$logdir/zrdn0_log.txt
	...
	then
		for killed_targets in $Targets_kill_list
		do
			echo -e "ZRDN 0: \e[32mуничтожена \e[0mЦель ID:$killed_targets"
			#в лог
			echo -e "ZRDN 0: \e[32mуничтожена \e[0mЦель ID:$killed_targets" >>$logdir/zrdn0_log.txt
			rm try_kill_zrdn0/$killed_targets #убираем из папки истинно уничтоженные цели
		done
	fi
