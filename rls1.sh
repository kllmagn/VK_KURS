...
target_counter=0
#--------log files--------
rm $logdir/rls1_log.txt 2>/dev/null
touch $logdir/rls1_log.txt

rm $logdir/rls1_status.txt 2>/dev/null
touch $logdir/rls1_status.txt
#----------------------------------------
...
do
	...
	do
		...
		echo imok >$logdir/rls1_status.txt
		...
		if (($gen_run != 0))
		then
			echo "процесс GenTarget убит. Завершаю rls1"
			exit 0
		fi
		...
		then
			...
			then
                ...
                then
					RLS1Detection ${target_params[$x_coord_index+$number_of_params*$target_index]} ${target_params[$y_coord_index+$number_of_params*$target_index]}
				fi
