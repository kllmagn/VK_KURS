...
target_counter=0
#--------log files--------
rm $logdir/rls2_log.txt 2>/dev/null
touch $logdir/rls2_log.txt

rm $logdir/rls2_status.txt 2>/dev/null
touch $logdir/rls2_status.txt
#----------------------------------------
...
do
	...
	do
		...
		echo imok >$logdir/rls2_status.txt
		...
		if (($gen_run != 0))
		then
			echo "процесс GenTarget убит. Завершаю rls2"
			exit 0
		fi
		...
		then
			...
			then
                ...
                then
					RLS2Detection ${target_params[$x_coord_index+$number_of_params*$target_index]} ${target_params[$y_coord_index+$number_of_params*$target_index]}
				fi
